...
{
	static CP_BOOL doInit = TRUE;
	static CP_BOOL isFullscreen = FALSE;
	static int windowWidth = 0;
	static int windowHeight = 0;
	if (doInit)
	{
		windowWidth = CP_System_GetWindowWidth();
		windowHeight = CP_System_GetWindowWidth();
		doInit = FALSE;
	}
	...
	if (CP_Input_KeyTriggered(KEY_F))
	{
		...
		{
			CP_System_Fullscreen();// Advanced(windowWidth, windowHeight);
			isFullscreen = TRUE;
		}
	}
	CP_Settings_RectMode(CP_POSITION_CORNER);
