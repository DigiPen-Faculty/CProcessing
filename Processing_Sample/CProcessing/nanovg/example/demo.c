...
void drawEyes(NVGcontext* vg, float x, float y, float w, float h, float mx, float my, float t)
{
	...
	float blink = 1.0f - (float)pow(sinf(t*0.5f),200)*0.8f;
...
void drawParagraph(NVGcontext* vg, float x, float y, float width, float height, float mx, float my)
{
	...
	nvgBeginPath(vg);
	nvgFillColor(vg, nvgRGBA(220,220,220,255));
	nvgRoundedRect(vg, bounds[0]-2,bounds[1]-2, (bounds[2]-bounds[0])+4, (bounds[3]-bounds[1])+4, 3);
...
static void unpremultiplyAlpha(unsigned char* image, int w, int h, int stride)
{
	...
	for (y = 0; y < h; y++) {
		...
		for (x = 0; x < w; x++) {
			...
			if (a != 0) {
				row[0] = (unsigned char)(int)mini(r*255/a, 255);
				row[1] = (unsigned char)(int)mini(g*255/a, 255);
				row[2] = (unsigned char)(int)mini(b*255/a, 255);
			}
			row += 4;
		}
	}
