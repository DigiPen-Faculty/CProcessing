...
static NVGcompositeOperationState nvg__compositeOperationState(int op)
{
	...
	{
		sfactor = NVG_ZERO;
		dfactor = NVG_SRC_COLOR;
	}
	else if (op == NVG_BLEND_SUBTRACT)
	{
		equation = NVG_BLEND_EQUATION_SUBTRACT;
		sfactor = NVG_ONE;
		dfactor = NVG_ONE;
	}
	else if (op == NVG_BLEND_MIN)
	{
		equation = NVG_BLEND_EQUATION_MIN;
		sfactor = NVG_ONE;
		dfactor = NVG_ONE;
	}
	else if (op == NVG_BLEND_MAX)
	{
		equation = NVG_BLEND_EQUATION_MAX;
		sfactor = NVG_ONE;
		dfactor = NVG_ONE;
	}
	else
...
{
	NVGstate* state = nvg__getState(ctx);
	state->textureFilterMode = filterMode;
}
